...
- **液态玻璃质感与环境自适应（v2.8.0，动 `liquid-glass.js` / main.css 玻璃区前必读）**：① 质感参数 `--glass-satur`/`--glass-bright` 是单一来源（.theme-light 定义，main.css 5 处 backdrop 与 liquid-glass.js 滤镜链/内联同读），**新增玻璃面禁止再写死 saturate/brightness 字面量**（test-features 有断言）；② 噪点层（feTurbulence+soft-light）只在 full 档滤镜链（aberration 分支），别挪到 standard——性能护栏刻意不放开；③ 分层模糊 `--glass-blur-soft`（×0.6）用于卡片组，侧栏保持全量 `--glass-blur`；④ 暗色玻璃 token 已删（恒浅色是产品决策），别加回 `:root` 死代码；⑤ 环境自适应（电池→frost + 亚克力→mica；系统透明开关关→frost）是**会话级**：渲染层 liquid-glass.js 用 `setMode(eff, {persist:false})`，**绝不能写 localStorage**，主进程材质降级在窗口最大化时跳过（27H2 风险），恢复走 `on-ac`/事件重放；⑥ 失焦纱用 `body.win-inactive::after` 覆盖层（z 9998），**不要用 body opacity**——隔离组会破坏 backdrop-filter 采样。
- **内嵌壁纸轮换（v2.8.0）**：5 张实拍壁纸在 `src/assets/bg/wp-*.jpg`（随包 asar 分发，CSS `data-preset-bg^="wp-"` 接管 `.layout::before`），轮换引擎在 pathbinding.js（`WP_LIST` 顺序即轮换顺序）：只切 `data-preset-bg` 属性值、选择器同步跟随；计时器对渐变/无背景空转、`visibilitychange` 隐藏即停；状态存 winclean-appearance（wallpaperRotate/wallpaperInterval）。第三方注入类工具检测（main.js `detectDwmInjectTools`，启动 12s 后一次性 tasklist+schtasks）结果经 `diag:dwm-conflict` 只读通道回渲染层，UI 文案只描述类别**不含第三方品牌名**（检测用的进程名/任务名字符串是功能本体，保留）。
