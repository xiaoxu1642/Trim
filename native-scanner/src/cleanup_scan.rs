...
fn fatal(msg: &str) -> ! {
    ...
    eprintln!("[cleanup-scan] 致命: {}", msg);
    std::process::exit(2);
}

/// argv = [categories_json, configured_paths_json]；规则 JSON 从 stdin 读入
    ...
    for cat in &categories {
        ...
        if rule.get("fileKeys").map(|f| f.ps_count()).unwrap_or(0) > 0 {
            ...
            for (fp, size) in &st.files {
                println!("@@PLANFILE@@{{\"id\":{},\"path\":{},\"size\":{}}}", jstr(cat_id), jstr(fp), size);
            }
            flush_stdout();
            continue;
        }
        ...
        let evaluated = match resolve_rule_path(expr) {
            Some(p) => p,
            None => {
                eprintln!("[cleanup-scan] [DIAG] stage=scan.pathPs mutation=skip detail={} pathPs 表达式不符合受限语法，已拒绝求值", cat_id);
                continue;
            }
        };
