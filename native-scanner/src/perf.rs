...
fn bench_progress(phase: &str, percent: u64) {
    let _ = out_line(&format!("__PROG__{{\"phase\":\"{}\",\"percent\":{}}}", phase, percent.min(100)));
}
...
pub fn run_diskbench(args: &[String]) -> i32 {
    ...
    let dir = match arg_value(args, "path") {
        ...
        None => { let _ = out_line("缺少 --path"); return 2; }
    };
    ...
    if !BENCH_BLOCKS.contains(&block) || !BENCH_DURATIONS.contains(&duration) {
        let _ = out_line("非法参数（block/duration 不在白名单）");
        return 2;
    }
...
pub fn run_net_sample(args: &[String]) -> i32 {
    ...
    let mut prev: std::collections::HashMap<u32, (u64, u64, u64)> = std::collections::HashMap::new();
    loop {
        unsafe {
            ...
            if GetIfTable2(&mut table) == 0 && !table.is_null() {
                ...
                if !adapters.is_empty() {
                    let line = format!("{{\"t\":{},\"adapters\":[{}]}}", now, adapters.join(","));
...
pub fn run_mem_clean(args: &[String]) -> i32 {
    ...
    if want.is_empty() { let _ = out_line("未选择要清理的内存区域"); return 2; }
    for id in &want {
        if !MEM_ITEMS.iter().any(|m| m.0 == id.as_str()) {
            let _ = out_line(&format!("非法区域 id: {}", id)); // fail-closed：白名单外拒绝
            return 2;
        }
    }
    unsafe {
